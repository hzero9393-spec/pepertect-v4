/**
 * ═══════════════════════════════════════════════════════════════════════════
 * Auto-Exit Worker — Standalone Background Monitoring Engine
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Standalone version for the WebSocket + REST backend server.
 * Single-process singleton — no globalThis HMR guard needed.
 *
 * Architecture:
 ...
 *   5. Notifies subscribers (WebSocket) of exits
 *
 * Fail-Safe:
...
import { calculateBrokerage } from '../lib/trade-utils.js'
import { Prisma } from '@prisma/client'
import { getOptionChainManager } from './option-chain-manager.js'
...
class AutoExitWorker {
  ...
  private idleQuiet = false  // Suppresses per-cycle logs when no SL/Target positions exist
  ...
  /** Subscribe to exit events (for WebSocket push) */
  ...
  private start() {
    ...
    logger.info('[AutoExit Worker] Starting auto-exit engine (500ms interval)')
  ...
  private async checkShouldStop() {
    try {
      ...
      if (count === 0 && !this.idleQuiet) {
        logger.info('[AutoExit Worker] No SL/Target positions — pausing logging (will resume on next trade)')
        this.idleQuiet = true
      } else if (count > 0) {
        this.idleQuiet = false
      }
  ...
  private async runCycle() {
    ...
    try {
      ...
      // Log every 60th cycle (~30 seconds) when not idle
      if (cycleNum % 60 === 0 && !this.idleQuiet) {
        logger.info(`[AutoExit Worker] Cycle ${cycleNum}: monitoring ${allPositions.length} positions with SL/Target`)
      }

    } catch (err) {
      logger.error(`[AutoExit Worker] [ERROR] Cycle ${cycleNum}: ${err instanceof Error ? err.message : err}`)
    }
  }
    ...
    // OPTIONS → cache → OC manager → DB
      ...
      const option = await db.option.findFirst({
        where: {
          ...
          ...(position.expiryDate ? { expiryDate: new Date(position.expiryDate) } : {}),
          isActive: true,
        },
        orderBy: { expiryDate: 'asc' },
        select: { ltp: true },
  ...
  destroy() {
    ...
    logger.info('[AutoExit Worker] Destroyed')
  }
}

// ─── Singleton Factory (no globalThis HMR guard — standalone process) ─────

export function getAutoExitWorker(): AutoExitWorker {
  return AutoExitWorker.getInstance()
}